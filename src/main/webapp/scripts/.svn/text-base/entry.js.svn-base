...
$(function(){
	...
	var msg = "不能为空";
	$("#entryForm").validate({
		rules:{entryDate:{required:true},bank:{required:true},accountNo:{required:true},cell:{required:true},censusType:{required:true},idcard:{required:true},checkbox:{required:true}},
	...
	$('#zgrid').jqGrid({
		...
		height:250,
		width:790,
	   	colNames:['ID','traceId','processId','node','头像','部门','职务','工号','英文名','中文名','称谓','出生年月','户籍','民族','手机','身份证','身份证附件','开户行','账号'],
	   	colModel:[
		...
		onSelectRow:function(ids) {
			...
			$('#agrid').jqGrid('setGridParam',{url:"get_user_edu.htm?t=ADR&id="+ids,page:1});
...
$('#zgrid').jqGrid('navGrid','#zpager',{add:false,editfunc:editUserDlg,del:false,search:false,alerttext:"请选择需要操作的数据行!"},{},{},{},{caption:"查找", Find:"点击查找", multipleSearch:true},{});
	...
	$('#agrid').jqGrid({
		...
		width:790,
		datatype:'json',
	   	colNames:['ID','UID','联系人','关系', '手机','电话','地址类型','地址', '邮编'],
	   	colModel:[
			...
			{name:'uid',index:'uid',hidden:true,editable:true},
	   		...
	   		{name:'cell',index:'cell',width:100,editable:true,editoptions:{size:14},editrules:{custom:true,custom_func:checkCell}},
		...
		gridComplete:function(){
			$('#egrid').jqGrid('setGridParam',{url:"get_user_edu.htm?t=EDU&id="+$('#id').val(),page:1});
	...
	$('#egrid').jqGrid({
		...
		width:790,
		datatype:'json',
	   	colNames:['ID','UID','附件','开始日期','结束日期', '学历', '毕业学校','专业','证书编号'],
	   	colModel:[
	   	    {name:'id',index:'id',hidden:true},
...
var editUserDlg = function() {
	 ...
	 dlg.find('#processId').val(rd.posId);
	 dlg.find('#node').val(rd.pwd);
	 dlg.find('#name').empty();
...
var editUser = function() {
  ...
  if($('#entryForm').valid() && dlg.find('#approval').attr('checked')){
	  ...
	  $.ajax({url:'save_entry.htm',type:'POST',data:$('form').serialize(),dataType:'json',cache:false,success:function(data){
 ...
 else alert("没上传证件的请先上传，且其他必填选项均不能为空");
};
...
var addEdu = function() {
	var dlg = $('#content_edu');
	var params = {id:$("#id").val(),uid:$('#uid').val(),idScan:$("#portrait").val(),startDate:$.trim(dlg.find("#startDate").val()),endDate:$.trim(dlg.find("#endDate").val()),ed:$.trim(dlg.find("#ed").val()),school:$.trim(dlg.find("#school").val()),major:$.trim(dlg.find("#major").val()),idno:$.trim(dlg.find("#idno").val())};
	  //验证方法
	  if(!valiEdu(params)) {
...
else if(re.test(value)) return [true,""]; 
else return [false,"手机号输入有误"];
} 

function checkPhone(value,colname) {
if(value.length > 9) {if(/^[0][1-9]{2,3}-[0-9]{5,8}$/.test(value)) return [true,""];else return [false,"电话输入有误"];}
else{if(/^[1-9]{1}[0-9]{5,8}$/.test(value))return [true,""];else return [false,"电话输入有误"];}
}
