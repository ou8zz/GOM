...
$(function(){
	...
	$('#asset_tb').jqGrid('navGrid','#asset_pg',{addfunc:addDlg,editfunc:editDlg,delfunc:delDlg,search:false,alerttext:"请选择需要操作的数据行!"},{},{},{},{caption:"查找", Find:"点击查找", multipleSearch:false},{});
...
var addAsset = function() {
	...
	//验证表单
	if(!dlg.find("#assetForm").valid()) return false;
	
	//验证采购人员
	//if(!isBuyer(params.buyer)) return false;
...
var delAsset = function() {
	 var dlg = $("#gConsole");
	 var tid = jQuery("#asset_tb").jqGrid('getGridParam','selrow');
	 if(tid) {
	    $.post('../del_asset.htm',{'id':tid},function(data){
		  if(data.result=="SUCCESS") {
		    $('#asset_tb').jqGrid("delRowData",tid);
			dlg.dialog("close");
			alert("删除成功！");
		  }
		  else alert("删除操作失败！");
