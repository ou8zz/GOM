...
var caption = {zh: "异常列表", "zh_CN": "异常列表<select id='abtype' onchange='javascript:search(this)'><option value='sh'>需要审核</option><option value=''>全部</option></select>", "zh_TW": "異常列表<select id='abtype' onchange='javascript:search(this)'><option value='sh'>需要审核</option><option value=''>全部</option></select>"};
...
$(function(){
	...
	//显示异常的列表
	initGrid("get_abnormals.htm?sh=sh", true);
	jQuery("#zgrid").navGrid('#zpager', {add:false, editfunc:shenheDlg, edittitle:"审核", editicon:"ui-icon-note", delfunc:editDlg, deltitle:"提交给下个负责人", delicon:"ui-icon-circle-triangle-e", search:false}).jqGrid('navButtonAdd','#zpager',{caption: "查看流程", onClickButton:viewProcess});
...
var search = function(obj) {
	...
	$("#zgrid").GridUnload();
	if(key=="") {
		initGrid("get_abnormals.htm?sh=" + key, false);
		jQuery("#zgrid").navGrid('#zpager', {add:false, edit:false, edittitle:"审核", editicon:"ui-icon-note", del:false, deltitle:"提交给下个负责人", delicon:"ui-icon-circle-triangle-e", search:false}).jqGrid('navButtonAdd','#zpager',{caption: "查看流程", onClickButton:viewProcess});
	} else {
		initGrid("get_abnormals.htm?sh=" + key, true);
		jQuery("#zgrid").navGrid('#zpager', {add:false, editfunc:shenheDlg, edittitle:"审核", editicon:"ui-icon-note", delfunc:editDlg, deltitle:"提交给下个负责人", delicon:"ui-icon-circle-triangle-e", search:false}).jqGrid('navButtonAdd','#zpager',{caption: "查看流程", onClickButton:viewProcess});
	}
	$("#abtype").val(key);
};
...
var shenheDlg = function() {
	...
	if(selectrow) {
		...
		panel.find("button:contains('拒绝')").show();
...
//流程操作（批量）
var traceAbnormal = function(status) {
	var selectrow = $("#zgrid").jqGrid("getGridParam", "selarrrow");
	var traceIds = new Array();
	for(var i in selectrow) {
		var rd = $('#zgrid').jqGrid("getRowData", selectrow[i]);
		traceIds[i] = rd.traceId;
	}
	
	$.blockUI({message:'<h6><img src="../images/ztree/loading.gif" /> 系统正在处理中，请稍候！</h6>',css:{border:'none',padding:'15px',backgroundColor:'#000', '-webkit-border-radius':'10px', '-moz-border-radius':'10px', opacity:.5, color:'#fff' }}); 
	
	$.post('save_abnormal.htm', {'ids':selectrow, 'traceIds':traceIds, 'status':status }, function(data) {
		if (data.result == "SUCCESS") {
			alert("操作成功");
			//for(var i in selectrow) {alert(selectrow[i]);$('#zgrid').jqGrid("delRowData", selectrow[i]);}
			$("#zgrid").GridUnload();
			initGrid("get_abnormals.htm?sh=sh");
			$("#content_main").dialog("close");
		}
		else if(data.result == "PARAM_EMPTY") alert("您不能操作这个项或该项已经审核完成");
		else alert("操作失败");
	}, 'json');
};

var initGrid = function(url, multi) {
	jQuery("#zgrid").jqGrid({
		height: 400,
		width:793,
	   	url:url,
